...
def load_live(day):
    ...
    path = ROOT / f"直播明细_全部账号_{ymd}_{ymd}.xlsx"
    if not path.exists():
        print(f"Warning: {path} not found, returning empty live data")
...
def load_live_detail(day, account_name):
...
def load_compass(day):
    ymd = day.strftime("%Y%m%d")
    path = ROOT / f"抖音电商罗盘-成交分析-{ymd}-{ymd}.xlsx"
    if not path.exists():
        print(f"Warning: {path} not found, returning empty compass data")
        return {}
    wb = load_workbook(path, data_only=True, read_only=True)
    ws = wb["自营成交"]
    ws.reset_dimensions()
...
def build_writes(day, offline_example=False):
    serial = excel_serial(day)
    day_num = day.day
    live = load_live(day)
    compass = load_compass(day)
    ...
    # 阴山优麦冲饮旗舰店直播间数据记录表
    records = load_live_detail(day, LIVE_ACCOUNT)
    if records:
...
def main():
    ...
    writes = build_writes(day, offline_example=args.offline_example)
    if args.mode == "plan":
